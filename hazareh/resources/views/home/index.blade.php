...
{{-- ══ HERO SLIDER ══════════════════════════════════════════ --}}
<section class="hero-slider" aria-label="تصاویر اسلایدر">
    @foreach($sliders as $slider)
    <div class="slide {{ $loop->first ? 'active' : '' }}">
        <img src="{{ asset('storage/sliders/' . $slider->image) }}"
             alt="{{ $slider->title }}" loading="{{ $loop->first ? 'eager' : 'lazy' }}">
        <div class="slide-content">
            <h2 class="slide-title">{{ $slider->title }}</h2>
            @if($slider->subtitle)
            <p class="slide-sub">{{ $slider->subtitle }}</p>
            @endif
        </div>
    </div>
    @endforeach

    <button class="slider-arrow prev" aria-label="قبلی"><i class="ti ti-chevron-right"></i></button>
    <button class="slider-arrow next" aria-label="بعدی"><i class="ti ti-chevron-left"></i></button>

    <div class="slider-controls">
        @foreach($sliders as $slider)
        <button class="slider-dot {{ $loop->first ? 'active' : '' }}" aria-label="اسلاید {{ $loop->iteration }}"></button>
        @endforeach
    </div>
</section>
...
<div class="stats-bar">
    <div class="container">
        <div class="stats-grid">
            <div class="stat-item">
                <div class="stat-number">۶۹</div>
                <div class="stat-label">هنرجوی فعال</div>
            </div>
            <div class="stat-item">
                <div class="stat-number">۳</div>
                <div class="stat-label">رشته تخصصی</div>
            </div>
            <div class="stat-item">
                <div class="stat-number">۳</div>
                <div class="stat-label">همکار صنعتی</div>
            </div>
            <div class="stat-item">
                <div class="stat-number">۱۴۰۴</div>
                <div class="stat-label">سال تأسیس</div>
            </div>
        </div>
    </div>
</div>
...
<section class="section section-gray" id="location">
    <div class="container">
        ...
        <div style="display:grid; grid-template-columns:1fr 1.5fr; gap:1.5rem; align-items:stretch;">
            ...
            <div style="border-radius: var(--radius-lg); overflow:hidden; min-height:300px;">
                <iframe
                    src="https://maps.google.com/maps?q=هنرستان+هزاره+صنعت&output=embed&hl=fa"
                    width="100%" height="100%" style="border:0; min-height:300px;"
                    allowfullscreen loading="lazy" title="موقعیت هنرستان">
                </iframe>
            </div>
        </div>
    </div>
